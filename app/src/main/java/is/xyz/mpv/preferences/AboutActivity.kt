    ...
    companion object {
        ...
        private val CHANGELOG = """
            ...
            [v73] 임베드 메타 조회 자동 재시도 (52)
             ...
             · fetchSync 실패 시 0.6s 후 1회 재시도(rows 비면=진짜 없음, 재시도 안 함)

            [v72] 임베드 안정화 (52)
             · hub 메타 조회 타임아웃 1.5s→8s — 다운로드 부하 시 receiver 응답이
               1.5s 를 넘겨 '메타 없음' 오판 → 단독/일괄 임베드 실패하던 것 수정
             · 날짜형 무수정 등 배우·스튜디오가 비어도 제목·날짜·커버는 정상 임베드

            [v71] 커버 보강 (52)
             · 메뉴 '커버 보강(없는 것만)' — 커버 없는 영상에 hub 커버 일괄 재임베드
             · 이미 커버 있는 파일은 건너뜀(파일 재작성 회피)
             · 다운로드 시점 커버 미상이던 작품의 FBI/프레임 썸네일 해결
             · 서버 일괄도구(reembed_covers.py)와 짝 — 내부 대량=서버, 외부SAF/선택=앱
