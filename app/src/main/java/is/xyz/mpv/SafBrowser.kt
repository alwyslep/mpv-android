...
class SafBrowserActivity : AppCompatActivity() {
    ...
    private val playLauncher =
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { res ->
            ...
            else if (u != null && Playback.shouldAdvance(this, u, entries.filter { !it.isDir }.find { it.uri.toString() == u }?.name)) {
    ...
    private fun setupToolbar() {
        val prefs = getSharedPreferences("media_library", MODE_PRIVATE)
        LibToolbar.build(toolbar, setOf("toggle", "sort", "tune", "heal", "dup", "refresh", "embedauto"), grid) { key ->
